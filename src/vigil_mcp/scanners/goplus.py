...
class GoPlusScanner:
    ...
    async def token_security(self, token: str, chain: str) -> GoPlusResult:
        ...
        return GoPlusResult(
            ...
            owner_change_balance=_b(row.get("owner_change_balance")),
            selfdestruct=_b(row.get("selfdestruct")),
            external_call=_b(row.get("external_call")),
            is_blacklisted=_b(row.get("is_blacklisted")),
            is_whitelisted=_b(row.get("is_whitelisted")),
