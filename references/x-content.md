...
An onchain security agent on Base — built as an MCP server.

6 tools. 1 protocol. Zero tolerance for scams.

→ Scan token approvals
→ Detect honeypots before you buy
→ Score any contract 0-100
...
Simulates buy AND sell before you commit.

Catches tokens that:
- Block selling
- Apply 99% sell tax
- Trap liquidity in contract
...
Every contract gets a 0-100 rating based on:
...
That means any AI client can use it.
...
Not multi-chain generic.
Purpose-built for Base L2 ecosystem.
...
PR submitted to @BankrBot skills.
...
That's exactly how drains happen.
...
Instead of guessing, run a VIGIL scan.

0-100 safety score based on:
Code + Ownership + Liquidity + Holders
...
Most wallets have unlimited approvals to contracts they forgot about.

One exploit = your entire balance.
...
VIGIL is a security MCP server on Base.

Any AI client becomes a blockchain security expert.
...
- Detects honeypots
- Scores contracts
- Revokes dangerous approvals

All on Base. All open source.
